...
GLvoid TgMeshBuilder::createCube()
{
    // Add vertices
...
GLvoid TgMeshBuilder::createGrid()
{
	// Get the dimensions
...
GLvoid TgMeshBuilder::createCylinder()
{
    // Insert vertices
    std::vector<std::size_t> idxA, idxB;
    for (std::size_t i = 0; i < m_nSectorCount; i++)
    {
    ...
    // Insert faces
    for (std::size_t j = 0; j < m_nSectorCount; j++)
    {
        // Draw sides
...
GLvoid TgMeshBuilder::createSphere()
{
    // Vertex position
    float x, y, z, xz;
...
GLvoid TgMeshBuilder::createIcoSphere()
{
    // constants
...
const TgMesh &TexGen::TgMeshBuilder::getMesh() const
{
    return m_mesh;
}
