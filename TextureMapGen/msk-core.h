...
{
	glm::vec3 v0, v1, v2;
	std::shared_ptr<MskEdge> edge;  // one of the half-edges emantating from the vertex 
};

struct MskFace2
{
	std::shared_ptr<MskEdge> edge; // one of the half-edges bordering the face
	glm::vec3 faceNormal;
};

struct MskEdge
{
	std::shared_ptr<MskVertex> vert; // vertex at the end of the half-edge 
	std::shared_ptr<MskEdge> pair; // oppositely oriented adjacent half-edge
	std::shared_ptr<MskFace2> face; // face the half-edge borders
	std::shared_ptr<MskEdge> next; // next half-edge around the face
};
...
struct MskMaterial : public MskObject {
public:
	glm::vec3 ambient;
	glm::vec3 diffuse;
	glm::vec3 specular;
...
struct MskMesh : public MskObject {
	...
	virtual GLvoid AddCyclinder() = 0;
	virtual GLvoid AddCone() = 0;
	...
	virtual GLvoid SetIndices(std::vector<GLuint>& indices) = 0;
...
struct MskMeshBuilder : public MskObject 
{
...
struct MskStencilRenderer : public MskObject
{
	virtual void Begin() = 0;
	virtual void End() = 0;
	virtual void Flush() = 0;
	virtual void Draw(std::function<void(int)> renderPass) = 0;
	virtual void Draw(std::shared_ptr<MskMesh>& mesh) = 0;
	virtual void SetProjectionMatrix(glm::mat4& matProj) = 0;
};
...
public:
	...
	Event<MouseEventArgs> MouseOver;
	Event<MouseEventArgs> MouseDoubleClick;
	...
	virtual GLvoid AddScene(const std::shared_ptr<MskScene>& scene) = 0;
	virtual void Show() = 0;
	virtual void Hide() = 0;
...
struct MskIntersectionInfo : public MskObject
{
public:
	std::shared_ptr<MskMesh> mesh;
	std::shared_ptr<MskFace> face;
	std::unique_ptr<glm::vec3> bary;
};

struct MskMousePicker : public MskObject
{
...
public:
	...
	virtual GLvoid CreateStencilRenderer(std::shared_ptr<MskStencilRenderer>& mskStencilRenderer) = 0;
	virtual GLint  Run() = 0;
