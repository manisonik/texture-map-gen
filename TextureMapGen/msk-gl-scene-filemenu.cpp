...
GLvoid MSceneFileMenu::InitMesh()
{
	...
	// Resize cube
	m_pMeshCube->AddCube();
	m_pMeshCube->SetColor(glm::vec4(0.0f, 1.0f, 1.0f, 1.0f));
	m_pMeshCube->Scale(1.0f, 1.0f, 1.0f);
	m_pMeshCube->Update();

	// Create cyclinder
	m_pMeshCyclinder->AddCyclinder();
	m_pMeshCyclinder->Scale(1.0f, 2.0f, 1.0f);
	m_pMeshCyclinder->Update();
}
...
void MSceneFileMenu::OnRender(MskObject* sender, EventArgs* e)
{
	...
	m_pStencilRenderer->Draw(std::bind(&MSceneFileMenu::RenderPass, this, std::placeholders::_1));
	...
	//m_pShapeRenderer->SetProjectionMatrix(m_pSceneCamera->GetModelViewProjectionMatrix());
	//m_pShapeRenderer->Draw(m_pMeshCyclinder);
	//m_pShapeRenderer->End();

	//m_pLineRenderer->Begin();
	//m_pLineRenderer->SetProjectionMatrix(m_pSceneCamera->GetModelViewProjectionMatrix());
	//m_pLineRenderer->Draw(m_pMeshCyclinder);
	//m_pLineRenderer->End();
...
void MSceneFileMenu::RenderPass(int pass) {
	if (pass == 0) {
		...
		m_pShapeRenderer->SetProjectionMatrix(m_pSceneCamera->GetModelViewProjectionMatrix());
		if (m_bHitTest) {
			m_pShapeRenderer->Draw(m_pMeshCube);
		}
		m_pShapeRenderer->End();
...
void MSceneFileMenu::OnMouseClick(MskObject* sender, MouseEventArgs* e)
{
...
void MSceneFileMenu::OnMouseMove(MskObject* sender, MouseEventArgs* e)
{
	...
	// Loop through meshes
	std::vector<MskFace> btnfaces = m_vButtons[0]->GetFaces();
	std::vector<glm::vec3> verts = m_vButtons[0]->GetVertices();

	m_bHitTest = 0;
	for (auto it = btnfaces.begin(); it != btnfaces.end(); ++it) {
		if (it->indices.size() % 3 == 0) {
			glm::vec3 v0 = glm::vec3(m_vButtons[0]->GetModelMatrix() * glm::vec4(verts[it->indices[0]], 1.0f));
			glm::vec3 v1 = glm::vec3(m_vButtons[0]->GetModelMatrix() * glm::vec4(verts[it->indices[1]], 1.0f));
			glm::vec3 v2 = glm::vec3(m_vButtons[0]->GetModelMatrix() * glm::vec4(verts[it->indices[2]], 1.0f));

			glm::vec3 v3 = glm::vec3(m_vButtons[0]->GetModelMatrix() * glm::vec4(verts[it->indices[3]], 1.0f));
			glm::vec3 v4 = glm::vec3(m_vButtons[0]->GetModelMatrix() * glm::vec4(verts[it->indices[4]], 1.0f));
			glm::vec3 v5 = glm::vec3(m_vButtons[0]->GetModelMatrix() * glm::vec4(verts[it->indices[5]], 1.0f));
	...
	m_bHitTest(false)
{
	...
	mskApp->CreateMesh(m_pMeshCyclinder);
	...
	MouseClick += EventHandler(this, &MSceneFileMenu::OnMouseClick);
}
